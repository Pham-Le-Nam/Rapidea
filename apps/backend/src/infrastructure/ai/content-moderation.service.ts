...
import {
    ContentModerationService as ContentModerationPort,
    ModerationDecision,
    ModerationFile,
} from '../../application/ports/content-moderation.service';

@Injectable()
export class ContentModerationService implements ContentModerationPort {
    private readonly apiKey = process.env.OPENAI_API_KEY;

    async moderate(file: ModerationFile, transcript = ''): Promise<ModerationDecision> {
        if (!this.apiKey) {
            return {
                status: 'NOT_SCANNED',
                score: null,
    ...
    private extractText(file: ModerationFile, transcript: string) {
