...
export class PrismaTagsRepository implements TagsRepository {
    ...
    async ensure(names: readonly string[]) {
        await this.prisma.$transaction(
            names.map((name) => this.prisma.tag.upsert({
                where: { name },
                update: {},
                create: { name },
            })),
    ...
    async replaceCourseTags(courseId: string, tags: any[], isSuggested: boolean) {
    ...
    async replacePostTags(postId: string, tags: any[], isSuggested: boolean) {
    ...
    async replaceFileTags(fileId: string, tags: any[], scores: Map<string, number>) {
        await this.prisma.fileTag.deleteMany({ where: { fileId } });
    ...
    completeTranscript(fileId: string, text: string, language?: string, durationSec?: number) {
    ...
    failTranscript(fileId: string, errorMessage: string) {
        return this.prisma.fileTranscript.update({
            where: { fileId },
            data: {
                status: 'FAILED',
    ...
    async updateEmbeddings(tags: { id: string }[], embeddings: number[][]) {
        await this.prisma.$transaction(
